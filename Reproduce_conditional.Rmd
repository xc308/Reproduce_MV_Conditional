...
construct_mats <- function(theta, model_num, whole_mesh = F) {
  ...
  S11  <- makeS(r = Dobsvec, var = theta[3], kappa = theta[5], nu = nu1) # obs level
  S2_1 <- makeS(r = Dobsvec, var = theta[4], kappa = theta[6], nu = nu2) # obs level  
  ...
  if (model_num %in% c(3, 4, 7, 8) | whole_mesh == T) {
    ...
    S12 <- S21 <- S11 * theta[9]        # obs lvl
    S22 <- S2_1 + S11 * theta[9]^2      # obs lvl
  }
  ...
  SY_true <- rbind(cbind(S11_true, S12_true), cbind(S21_true, S22_true)) %>% as("dgeMatrix")  # obs lvl, the same as proc lvl at 157 locations
  ...
  # SY: Sigma obs lvl, So: Sigma measurement error, Z: matrix of obs
}
...
In the function we allow the dropping of certain observations for cross-validation purposes. The indices of the observations we wish to drop are stored in the parameter `i`. If `i = NULL` then no observations are dropped. This argument is useful for cross-validation.
...
optim_loglik <- function(par, model_num, i = NULL) {
  optim(par = par, 
        ...
        control = list(trace = 6, 
                       pgtol = 0,       # projected gradient
                       parscale = rep(0.1, length(par)),   # why 0.1 times? not 1
...
cokrig <- function(X, i = NULL) {
  ...
  #SS is proc lvl
    ...
    Z <- X$Z
    Q <- chol2inv(chol(C %*% SS %*% t(C))) %>% as("dgeMatrix")  # proc to obs lvl
    mu_pred <- SS %*% t(C) %*% Q %*% Z %>% as.numeric()
    #var_pred <- diag(SS - SS %*% t(C) %*% Q %*% C %*% t(SS)) %*% as.numeric() 
...
```{r}
save(pred, file = paste0("./all_predictions.rda"))
```
...
```{r}
save(optim_est_sh_pars, file = "./Results/Shifted_Pars_est_results.rda")
```
