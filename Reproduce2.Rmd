...
if (model_num %in% c(3, 4, 7, 8)) {
  B <- theta[9] * bisq_B(h, delta = theta[11:12], r =  theta[10], area = Area, 
...
if (LK_analysis == 1) {
  ...
  save("fit_all_data", file = paste0("/Users/xchen/Dropbox/Learn/Reproduce_MV_Conditional/fit_all_data.rda"))
} else {
...
sh_pars_mats <- function(theta) {
  ...
  CY11 <- makeC(D = Dobsvec, Var = theta[9], Kappa = theta[3], nu = theta[4])
  CY22 <- makeC(D = Dobsvec, Var = theta[10], Kappa = theta[3], nu = theta[5])
  CY21 <- makeC(D = D21, Var = theta[6] * sqrt(theta[9] * theta[10]),
               Kappa = theta[3], nu = 0.5 * (theta[4] + theta[5]))
  CY12 <- t(CY21)
  
  CY <- cbind(rbind(CY11, CY21), rbind(CY12, CY22))
