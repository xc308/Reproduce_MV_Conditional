...
construct_mat <- function(theta, model_num, whole_mesh = F) {
  ...
  if (whole_mesh == T) {
    ...
    C12 <- B %*% C11     # proc lvl
    C21 <- t(C12)
    C22 <- C2_1 %*% C1  + crossprod(chol(C11)) %*% t(B)
  }
...
fit_all_models <- function(i) {
  ...
  fit.Model2_rev <- optim_loglik(par = fit.Model1_rev$par, model_num = 6, i)
  fit.Model3_rev <- optim_loglik(par = fit.Model2_rev$par, model_num = 7, i)
  fit.Model4_rev <- optim_loglik(par = fit.Model3_rev$par, model_num = 8, i)
...
C11_proc <- makeC(Dvec, Var = fit_all_data[[1]]$par[3], Kappa = fit_all_data[[1]]$par[5], nu = fit_all_data[[1]]$par[7])
