...
construct_mat <- function(theta, model_num, whole_mesh = F) {
  ...
  Imat <- function(n) {
    return(sparseMatrix(i = 1:n, j = 1:n, x = 1))
  } 
...
X3 <- construct_mat(theta = append_theta(theta = fit_all_data[[3]]$par, model_num = 3), model_num = 3, whole_mesh = T)

X4 <- construct_mat(theta = append_theta(theta = fit_all_data[[4]]$par, model_num = 4), 
...
C11_proc <- makeC(D = Dvec, Var = fit_all_data[[4]]$par[3], Kappa = fit_all_data[[4]]$par[5], nu = fit_all_data[[4]]$par[7])

isSymmetric(C11_proc) # [1] TRUE
ch_C11 <- chol(C11_proc)


B <- fit_all_data[[4]]$par[9] * bisq_B(h = h, delta = fit_all_data[[4]]$par[11:12],
                           r = fit_all_data[[4]]$par[10],
                           area = Area,
                           n1 = n1,
                           n2 = n2)
...
t <- ch_C11 %*% t(B)
crosst <- crossprod(ch_C11 %*% t(B))


Cz <- X4$CY + X4$Co
a <- C %*% Cz %*% t(C)
isSymmetric(a)  # [1] TRUE

eg <- eigen(a, only.values = T)
all(eg$values > 0) # [1] FALSE
which(eg$values <= 0)
ch_a <- chol(a)
