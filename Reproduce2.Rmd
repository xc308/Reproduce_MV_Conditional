...
#X2 <- construct_mat(theta = append_theta(theta = fit.Model2$par, model_num = 2),
#                   model_num = 2, whole_mesh = T)

X2 <- construct_mat(theta = append_theta(theta = fit_all_data5[[2]]$par, model_num = 2), model_num = 2, whole_mesh = T)


#X3 <- construct_mat(theta = append_theta(theta = fit.Model3$par, model_num = 3),
#                    model_num = 3, whole_mesh = T)


X3 <- construct_mat(theta = append_theta(theta = fit_all_data5[[3]]$par, model_num = 3), model_num = 3, whole_mesh = T)


#X4 <- construct_mat(theta = append_theta(theta = fit.Model4$par,                                         model_num = 4), 
...
Cz_3 <- X3$CY + X3$Co
isSymmetric(Cz_3)  # [1] TRUE


Cz_obs <- C %*% Cz_3 %*% t(C)
str(Cz_obs)
isSymmetric(Cz_obs)  # [1] TRUE
chol_Cz3 <- chol(Cz_obs)   # asMethod(object) : matrix is not symmetric [174,200]

ei_Czobs <- eigen(Cz_obs, only.values = T)
all(ei_Czobs$values > 0)  # [1] FALSE
all(ei_Czobs$values >= 0)  # [1] FALSE
which(ei_Czobs$values < 0)  # [1] 309 310 311 312 313 314

le

Q <- chol2inv(chol(C %*% Cz %*% t(C)))
...
C11_proc <- makeC(D = Dvec, Var = fit.Model2$par[3], Kappa = fit.Model2$par[5], nu = fit.Model3$par[7])

C11_p <- makeC(D = Dvec, Var = fit_all_data[[2]]$par[3], Kappa = fit_all_data[[2]]$par[5],
      nu = fit_all_data[[2]]$par[7])

ch_C11p <- chol(C11_p)

isSymmetric(C11_proc) # [1] TRUE
ch_C11 <- chol(C11_proc)
...
# mine fit_all_data2 and fit.Model2
...
fit_all_data[[2]]$par[3]   # [1] 6.775386
fit_all_data[[2]]$par[5]   # [1] 0.01132269
fit_all_data[[2]]$par[7]   # [1] 0.6020534

fit_all_data2[[2]]$par[3]  # [1] 1.045282
fit_all_data2[[2]]$par[5]  # [1] 0.007850412
fit_all_data2[[2]]$par[7]  # [1] 5.199892


fit.Model2$par[3]    # [1] 1.045282
fit.Model2$par[5]    # [1] 0.00785041
fit.Model2$par[7]    # [1] 5.199892


# his code in Rp_con.Rmd
fit_all_data3[[2]]$par[3]  # 1.045282
fit_all_data3[[2]]$par[5]  # 0.007850412
fit_all_data3[[2]]$par[7]  # 5.199892
